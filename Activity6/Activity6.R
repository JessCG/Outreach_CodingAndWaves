### Change the line below to add your maze to the drop down menu ###

# Maze choice - add a coma and your maze number in the ( )s
# e.g. c(101, 76, 303)
all_maze <- c(101)
...
# Load finish image
fname_finish = "./SupportFiles/finish.png"
img_finish <- readPNG(fname_finish)  # load runner
...
# Load wall color and size
pref <- read.csv(paste("Preferences_", maze, ".csv", sep = ""))

# Set up a square plot area
par(pty="s")
plot(1:pref$size, type = 'n', main = "Get to the finish flags!", 
     xlab = "x", ylab = "y", xlim = c(0, pref$size), ylim = c(0, pref$size), 
     axes = FALSE, asp = 1)

# Fix axes
axis(1, at = seq(0, pref$size))
axis(2, at = seq(0, pref$size))

# Add walls
segments(walls$x1, walls$y1, walls$x2, walls$y2,
         col = as.character(pref$color), lwd = 4)

# Add guiding dots
dots_x = rep(seq(0.5, pref$size-0.5, by = 1), times = pref$size)
dots_y = rep(seq(0.5, pref$size-0.5, by = 1), each = pref$size)
...
# for (game in 1:50){  # used to troubleshoot
  ...
  if (cross == 1){
    rasterImage(img_skull, 0, 0, pref$size, pref$size)  # skull image
  ...
  if (x_position2-0.5 == positions$x_end && y_position2-0.5 == positions$y_end){
    ...
    rasterImage(img_fireworks, 0, 0, pref$size, pref$size)  # fireworks image
    print("You won!")
    game <- 0
    next
